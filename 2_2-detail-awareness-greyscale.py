...
def generate_grayscale_tiles(num_tiles=10, tile_size=100):
    tiles = []
    brightness_values = []
    for i in range(num_tiles):
        brightness = int((i / (num_tiles - 1)) * 255)
        brightness_values.append(brightness)
        tile_array = np.ones((tile_size, tile_size), dtype=np.uint8) * brightness
...
mosaic_image = Image.new('L', (NUM_COLS*BLOCK_SIZE, NUM_ROWS*BLOCK_SIZE), color=255)
solution = x.value
            ...
            for s_idx, s in enumerate(TILE_SIZES):
                ...
                if i+s <= NUM_ROWS and j+s <= NUM_COLS and solution[c,i,j,s_idx] > 0.5:
                    ...
                    if not placed_blocks[i,j]:
                        ...
                        draw.rectangle([0,0,tile.size[0]-1,tile.size[1]-1], outline=0, width=1)
...
for i in range(NUM_ROWS):
    for j in range(NUM_COLS):
        if not placed_blocks[i,j]:
            mosaic_image.paste(Image.new('L', (BLOCK_SIZE,BLOCK_SIZE), 255),
